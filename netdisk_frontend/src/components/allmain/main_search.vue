...
export default {
  ...
  methods: {
    ...
    viewDetail(row) {
      ...
      if (row.type === "folder") {
        ...
        this.getFolder(row.folderId, this.belongUser, 0);
        this.getFile(this.belongUser, row.folderId, 0);
    ...
    getFolders(data) {
      this.$axios.post('http://localhost:8088/folder/searchfolder', data).then(res => {
        console.log(res.data);
        for (const x of res.data) {
          let time = new Date();
          if (!x["type"]) {
    ...
    getFiles(data) {
      this.$axios.post('http://localhost:8088/file/searchfile', data).then(res => {
        console.log(res.data);
        for (const x of res.data) {
          let time = new Date();
          if (!x["type"]) {
    ...
    delayLoad(searchmsg) {
      setTimeout(() => {
        ...
        this.findallByUseridAndMsg(searchmsg);
      }, 1000)
    },
    updatemg(row) {
      if (row.type === "folder") {
      ...
      this.getFolder(row.folderFather, this.belongUser, 0);
      this.getFile(this.belongUser, row.folderFather, 0);
      }, 1500);
    },
    shareFile() {
    ...
    findallByUseridAndMsg(searchmsg) {
      this.getFolders({ belongUser: this.belongUser, folderName: searchmsg, isdel: 0 });
      this.getFiles({ belongUser: this.belongUser, fileName: searchmsg, isDel: 0 });
    },
    closeVideo() {
      this.dialogVideoUrl = "";
    },
  ...
  beforeMount() {
    this.findallByUseridAndMsg();
  },
  mounted() {
    this.$store.watch(
      state => state.searchmsg,
      (newVal) => {
        this.delayLoad(newVal);
      }
    )
  }
}
